...
bool jo_write_jpg(const char *filename, int width, int height, int comp, int quality) {
	...
	if (!filename || !width || !height || comp > 4 || comp < 1 || comp == 2) {
		return false;
	}
...
int main(int argc, char* argv[])
{
    ...
    auto table_file = std::ifstream(argv[1]);
    int number;

    while (table_file >> number) {
        table.push_back(number);
    }
    table_file.close();

	jo_write_jpg(argv[2], width, height, comp, quality);
}
#endif